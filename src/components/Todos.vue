    ...
    <div class="card my-3">
      <div class="card-body">
        ...
        <div v-if="todayTodos.length > 0">
          <h5>Today</h5>
          <hr />
          <todo v-for="todo in todayTodos" :key="todo.id" :todo="todo"></todo>
        </div>
        <div v-if="tomorrowTodos.length > 0">
          <h5>Tomorrow</h5>
          <hr />
          <todo v-for="todo in tomorrowTodos" :key="todo.id" :todo="todo"></todo>
        </div>
        <div v-if="otherTodos.length > 0">
          <h5>Not scheduled</h5>
          <hr />
          <todo v-for="todo in otherTodos" :key="todo.id" :todo="todo"></todo>
        </div>
    ...
    <div class="card my-3">
      <div class="card-body">
        ...
        <div v-if="todosCompleted.length > 0">
          <todo v-for="todo in todosCompleted" :key="todo.id" :todo="todo"></todo>
        </div>
        <div v-else>
...
export default {
  ...
  computed: {
    ...
    otherTodos() {
      var todos = this.todos.filter((todo) => {
        return (
          !todo.completed && todo.date !== "Today" && todo.date !== "Tomorrow"
        );
      });
      return todos.sort((a, b) => b.createdAt - a.createdAt);
    },
